...
const Item = ({ item, onPress }) => (
  <Card style={styles.cardStyle} key={item._id}>
    <List.Item
      title={item.title}
    ...
    <View style={styles.bottomTags}>
      <Chip
        ...
        icon={chooseIcon(item.tags[0])}
        style={styles.chip}
        textStyle={{ color: chooseTagColor(item.tags[0]), ...styles.chipText }}
      >
        {item.tags[0]}
      </Chip>
      <Micon.Button
      ...
      >
        <Text style={styles.miconText}>{item.comments.length}</Text>
      </Micon.Button>
      <Micon.Button
        name="heart"
        color="pink"
      ...
      >
        <Text style={styles.miconText}>{item.likes}</Text>
      </Micon.Button>
    </View>
  </Card>
);
...
const MyPending = ({ route }) => {
  ...
  const DATA = myPendingFiltered?.sort(
          ...
          <Menu
            ...
            anchor={
              <Button mode="text" onPress={openMenu}>
                <Text style={{ color: theme.colors.accent }}>กรอง </Text>
                <Text> {selectedFilterTag}</Text>
              </Button>
            }
          >
            <Menu.Item
              title="แสดงทั้งหมด"
              onPress={() => handleSetFilter("แสดงทั้งหมด")}
            />
            <Menu.Item
              title="ปัญหาเรื่องเพศ"
              value="ปัญหาเรื่องเพศ"
              onPress={() => handleSetFilter("ปัญหาเรื่องเพศ")}
            />
            <Menu.Item
              title="การเสพติด"
              value="การเสพติด"
              onPress={() => handleSetFilter("การเสพติด")}
            />
            <Menu.Item
              title="เพื่อน"
              value="เพื่อน"
              onPress={() => handleSetFilter("เพื่อน")}
            />
            <Menu.Item
              title="lgbt"
              value="lgbt"
              onPress={() => handleSetFilter("lgbt")}
            />
            <Menu.Item
              title="โรคซึมเศร้า"
              value="โรคซึมเศร้า"
              onPress={() => handleSetFilter("โรคซึมเศร้า")}
            />
            <Menu.Item
              title="ความวิตกกังวล"
              value="ความวิตกกังวล"
              onPress={() => handleSetFilter("ความวิตกกังวล")}
            />
            <Menu.Item
              title="ไบโพลาร์"
              value="ไบโพลาร์"
              onPress={() => handleSetFilter("ไบโพลาร์")}
            />
            <Menu.Item
              title="relationships"
              value="relationships"
              onPress={() => handleSetFilter("relationships")}
            />
            <Menu.Item
              title="การทำงาน"
              value="การทำงาน"
              onPress={() => handleSetFilter("การทำงาน")}
            />
            <Menu.Item
              title="สุขภาพจิต"
              value="สุขภาพจิต"
              onPress={() => handleSetFilter("สุขภาพจิต")}
            />
            <Menu.Item
              title="การรังแก"
              value="การรังแก"
              onPress={() => handleSetFilter("การรังแก")}
            />
            <Menu.Item
              title="ครอบครัว"
              value="ครอบครัว"
              onPress={() => handleSetFilter("ครอบครัว")}
            />
            <Menu.Item
              title="อื่นๆ"
              value="อื่นๆ"
              onPress={() => handleSetFilter("อื่นๆ")}
            />
            <Menu.Item
              title="ความรัก"
              value="ความรัก"
              onPress={() => handleSetFilter("ความรัก")}
            />
          </Menu>
        </View>
